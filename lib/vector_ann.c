...
void init_weights(struct ann *net){
    ...
    weights = (float***) volk_malloc(sizeof(float*)*(num_layers-1), alignment);

    for(size_t i=0; i<num_layers-1; i++){ // go through layers
        weights[i] = (float**) volk_malloc(sizeof(float*)*num_nodes[i], alignment);
        for(size_t j=0; j<num_nodes[i]; j++){ // go through this layer and allocate next layer
            weights[i][j] = (float*) volk_malloc(sizeof(float)*num_nodes[i+1], alignment);
        }
    }
    ...
    for(size_t i=0; i<num_layers-1; i++){ // go through layers
        for(size_t j=0; j<num_nodes[i]; j++){ // go through this layer
            for(size_t k=0; k<num_nodes[i+1]; k++){ // go through next layers
                weights[i][j][k] = rndm();
            }
        }
    }
